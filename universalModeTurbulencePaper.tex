...
$\Phi(t,\vect{r})$ is the electrostatic potential, $h_s(t,\vect{R}_s,\energy,\mu)$ is the nonadiabatic distribution function,
$\energy = v^2/2$, $\mu = v_{\bot}^2/(2B)$,
and $f_{Ms}$ is the leading-order Maxwellian.
The scale length of the equilibrium density $n$ is $L_n = -(1/n)(dn/dx)$,
...
and $h_s = \sum_{k_x,k_y} h_{s,k_x,k_y}(t,z,\energy,\mu) \exp(i k_x X_s + i k_y Y_s)$,
...
$L_s / L_n$, $\eta_i$, $\eta_e$, $T_e / T_i$, and $k_y \rho_i \nu_{ee} / \omega_*$,
where $\nu_{ee} = \sqrt{2}\pi n e^4 \ln\Lambda/[(4\pi\epsilon_0)^2 m_e^{1/2} T_e^{3/2}]$ is the electron-electron collision frequency,
